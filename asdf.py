...
reading=Falsefrom binance.client import Client
import numpy as np
import config
import sys
import time
...
#	atrShout=np.append(atrShout,atrListen())
	...
	if(len(quoteBase_close)>0):
		if(accountBalanceBase+(accountBalanceQuote*quoteBase_close[len(quoteBase_close)-1])<=minAmount or (accountBalanceBase+(accountBalanceQuote*quoteBase_close[len(quoteBase_close)-1]))<=stopPercent):
